...
handleEvents :: (MonadIO m, MonadReader Config m, MonadState WorldState m) => m ()
handleEvents = do
  game    <- get
  board   <- use board
  current <- use current
...
selectPiece newSel newcursor = do
  ...
  -- if new selection
          ...
          Right p -> if pcolor p == curr then do
                        let piece = show(pcolor p) ++" "++ show(ptype p) 
                        cursor  .= newcursor
                        message .= piece ++" selected"
                     else 
                        message .= "invalid selection! it is "++show(curr)++"'s turn"                             
...
makemove to = do
  ...
  if isCheck curr newBoard then
    message .= "invalid move! You are putting your King in Check-Mate"
  else do
    cursor .= Cursor to Nothing []  -- reset cursor
    current %= other                -- change turn
    
    -- get new turn color
    newp <- use current -- get 
    board  .= resetTwoStep newp newBoard    
    -- update board (and change Pawns in TwoStep State -> Moved) 
    
    -- did the move put new player in check?
    if isCheck newp newBoard then
      message .= "Atention! You are Check!"
    else
      message .= "select piece" 
